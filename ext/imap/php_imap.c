...
/* the gets we use */
static char *php_mail_gets(readfn_t f, void *stream, php_uint_t size, GETS_DATA *md);
...
static void php_imap_do_open(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	php_size_t mailbox_len, user_len, passwd_len;
	php_int_t retries = 0, flags = NIL, cl_flags = NIL;
...
PHP_FUNCTION(imap_reopen)
{
	zval *streamind;
	char *mailbox;
	php_size_t mailbox_len;
	php_int_t options = 0, retries = 0;
	pils *imap_le_struct;
	php_int_t flags=NIL;
	php_int_t cl_flags=NIL;
...
PHP_FUNCTION(imap_append)
{
	...
	php_size_t folder_len, message_len, internal_date_len = 0, flags_len = 0;
	pils *imap_le_struct;
	STRING st;
...
PHP_FUNCTION(imap_setacl)
{
	...
	php_size_t mailbox_len, id_len, rights_len;
	pils *imap_le_struct;
...
PHP_FUNCTION(imap_headers)
{
	...
	for (msgno = 1; msgno <= imap_le_struct->imap_stream->nmsgs; msgno++) {
		...
		tmp[22] = ' ';
		tmp[23] = '\0';
		mail_fetchfrom(tmp+23, imap_le_struct->imap_stream, msgno, (php_int_t)20);
		strcat(tmp, " ");
...
PHP_FUNCTION(imap_renamemailbox)
{
	...
	php_size_t old_mailbox_len, new_mailbox_len;
	pils *imap_le_struct;
...
PHP_FUNCTION(imap_listscan)
{
	...
	php_size_t ref_len, pat_len, content_len;
	pils *imap_le_struct;
	STRINGLIST *cur=NIL;
...
PHP_FUNCTION(imap_headerinfo)
{
	...
	php_size_t defaulthost_len = 0;
	int argc = ZEND_NUM_ARGS();
	php_int_t msgno, fromlength, subjectlength;
	pils *imap_le_struct;
	MESSAGECACHE *cache;
	ENVELOPE *en;
...
PHP_FUNCTION(imap_rfc822_parse_headers)
{
	...
	ENVELOPE *en;
	php_size_t headers_len, defaulthost_len = 0;
	int argc = ZEND_NUM_ARGS();
...
PHP_FUNCTION(imap_rfc822_write_address)
{
	...
	php_size_t mailbox_len, host_len, personal_len;
	ADDRESS *addr;
	char *string;
...
PHP_FUNCTION(imap_rfc822_parse_adrlist)
{
	...
	php_size_t str_len, defaulthost_len;
	ADDRESS *addresstmp;
	ENVELOPE *env;
...
PHP_FUNCTION(imap_utf7_decode)
{
	...
	unsigned char c;
	php_size_t arg_len, inlen, outlen;
	enum {
...
PHP_FUNCTION(imap_setflag_full)
{
	zval *streamind;
	char *sequence, *flag;
	php_size_t sequence_len, flag_len;
	php_int_t flags = 0;
	pils *imap_le_struct;
...
PHP_FUNCTION(imap_sort)
{
	...
	php_size_t criteria_len, charset_len;
	php_int_t pgm, rev, flags = 0;
	pils *imap_le_struct;
...
PHP_FUNCTION(imap_fetch_overview)
{
	zval *streamind;
	char *sequence;
	php_size_t sequence_len;
	pils *imap_le_struct;
	zval *myoverview;
	char *address;
	php_int_t status, flags = Z_I(0);
	int argc = ZEND_NUM_ARGS();
...
PHP_FUNCTION(imap_mail)
{
	...
	php_size_t to_len, message_len, headers_len, subject_len, cc_len, bcc_len, rpath_len;
	int argc = ZEND_NUM_ARGS();
...
PHP_FUNCTION(imap_search)
{
	...
	php_size_t criteria_len, charset_len = 0;
	php_int_t flags = SE_FREE;
	pils *imap_le_struct;
	char *search_criteria;
	MESSAGELIST *cur;
	int argc = ZEND_NUM_ARGS();
...
PHP_FUNCTION(imap_mime_header_decode)
{
	...
	php_size_t str_len;
	php_int_t charset_token, encoding_token, end_token, end, offset=0, i;
	unsigned long newlength;
 ...
 */
static void build_thread_tree_helper(THREADNODE *cur, zval *tree, php_int_t *numNodes, char *buf)
{
	php_uint_t thisNode = *numNodes;

	/* define "#.num" */
	snprintf(buf, 25, "%pd.num", thisNode);

	add_assoc_long(tree, buf, cur->num);

	snprintf(buf, 25, "%pd.next", thisNode);
	if(cur->next) {
		(*numNodes)++;
	...
	snprintf(buf, 25, "%pd.branch", thisNode);
	if(cur->branch) {
		(*numNodes)++;
		add_assoc_long(tree, buf, *numNodes);
