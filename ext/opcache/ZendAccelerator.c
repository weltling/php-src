...
static char *(*accelerator_orig_zend_resolve_path)(const char *filename, zend_size_t filename_len TSRMLS_DC);
#endif
static void (*orig_chdir)(INTERNAL_FUNCTION_PARAMETERS) = NULL;
static ZEND_INI_MH((*orig_include_path_on_modify)) = NULL;

#if ZEND_EXTENSION_API_NO < PHP_5_3_X_API_NO
static char *accel_php_resolve_path(const char *filename, zend_size_t filename_length, const char *path TSRMLS_DC);
#endif
...
static inline char* accel_getcwd(zend_size_t *cwd_len TSRMLS_DC)
{
	if (ZCG(cwd)) {
		*cwd_len = ZCG(cwd_len);
		return ZCG(cwd);
...
static const char *(*orig_new_interned_string)(const char *str, zend_size_t len, int free_src TSRMLS_DC);
 ...
 */
static const char *accel_new_interned_string_for_php(const char *str, zend_size_t len, int free_src TSRMLS_DC)
{
	return str;
}
...
char *accel_make_persistent_key_ex(zend_file_handle *file_handle, zend_size_t path_length, zend_size_t *key_len TSRMLS_DC)
{
    ...
    if (ZCG(accel_directives).use_cwd &&
        ...
        zend_size_t include_path_len = 0;
        const char *parent_script = NULL;
        zend_size_t parent_script_len = 0;
        zend_size_t cur_len = 0;
        zend_size_t cwd_len;
        char *cwd;
...
static inline char *accel_make_persistent_key(zend_file_handle *file_handle, zend_size_t *key_len TSRMLS_DC)
{
...
static void zend_accel_add_key(char *key, zend_size_t key_length, zend_accel_hash_entry *bucket TSRMLS_DC)
{
...
static zend_persistent_script *cache_script_in_shared_memory(zend_persistent_script *new_persistent_script, char *key, zend_size_t key_length, int *from_shared_memory TSRMLS_DC)
{
...
static zend_persistent_script *compile_and_cache_file(zend_file_handle *file_handle, int type, char *key, zend_size_t key_length, zend_op_array **op_array_p, int *from_shared_memory TSRMLS_DC)
{
...
static char* persistent_zend_resolve_path(const char *filename, zend_size_t filename_len TSRMLS_DC)
{
	if (ZCG(enabled) && accel_startup_ok &&
	    (ZCG(counted) || ZCSG(accelerator_enabled)) &&
	    !CG(interactive) &&
