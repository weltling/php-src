		...
		for (zend_hash_internal_pointer_reset(Z_ARRVAL_P(retval));
			...
			switch (hash_type) {
				case HASH_KEY_IS_LONG:
					key_length = slprintf(tmp, sizeof(tmp), ZEND_UINT_FMT, idx) + 1;
					key = tmp;
					/* fallthru */
 ...
 */
void php_wddx_packet_start(wddx_packet *packet, char *comment, php_size_t comment_len)
{
	...
	if (call_user_function_ex(CG(function_table), &obj, fname, &retval, 0, 0, 1, NULL TSRMLS_CC) == SUCCESS) {
		...
		for (zend_hash_internal_pointer_reset(objhash);
			...
			if (zend_hash_get_current_key_ex(objhash, &key, &key_len, &idx, 0, NULL) == HASH_KEY_IS_STRING) {
				...
				key_len = slprintf(tmp_buf, sizeof(tmp_buf), ZEND_UINT_FMT, idx);
 ...
 */
void php_wddx_serialize_var(wddx_packet *packet, zval *var, char *name, php_size_t name_len TSRMLS_DC)
{
	HashTable *ht;
 ...
 */
int php_wddx_deserialize_ex(char *value, php_size_t vallen, zval *return_value)
{
	wddx_stack stack;
	XML_Parser parser;
	st_entry *ent;
