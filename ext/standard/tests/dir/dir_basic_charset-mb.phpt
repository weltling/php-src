...
$dir_path = $file_path."/АБВГДЕЖЗИЙКЛМНОПFdir_basic";
@mkdir($dir_path);

// create files within the temporary directory
create_files($dir_path, 3, "alphanumeric", 0755, 1, "w", "АБВГДЕЖЗИЙКЛМНОПFdir_basic");
...
// delete temp files
delete_files($dir_path, 3, "АБВГДЕЖЗИЙКЛМНОПFdir_basic", 1, ".tmp");
echo "Done";
?>
--CLEAN--
<?php
...
object(Directory)#%d (2) {
  ["path"]=>
  string(%d) "%s/АБВГДЕЖЗИЙКЛМНОПFdir_basic"
  ["handle"]=>
...
object(Directory)#%d (2) {
  ["path"]=>
  string(%d) "%sАБВГДЕЖЗИЙКЛМНОПFdir_basic"
  ["handle"]=>
