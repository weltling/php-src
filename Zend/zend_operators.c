...
ZEND_API void _convert_to_string(zval *op ZEND_FILE_LINE_DC) /* {{{ */
{
	...
	switch (Z_TYPE_P(op)) {
		...
		case IS_RESOURCE: {
			...
			zend_list_delete(Z_LVAL_P(op));
			Z_STRLEN_P(op) = zend_spprintf(&Z_STRVAL_P(op), 0, "Resource id #" ZEND_INT_FMT, tmp);
			break;
		}
		case IS_LONG:
			lval = Z_LVAL_P(op);
			Z_STRLEN_P(op) = zend_spprintf(&Z_STRVAL_P(op), 0, ZEND_INT_FMT, lval);
			break;
		case IS_DOUBLE: {
			TSRMLS_FETCH();
			dval = Z_DVAL_P(op);
...
{
	zend_size_t length = Z_STRLEN_P(op1) + 1;
...
{
	zend_size_t length = Z_STRLEN_P(op1) + Z_STRLEN_P(op2);
	char *buf = str_erealloc(Z_STRVAL_P(op1), length + 1);
...
ZEND_API int concat_function(zval *result, zval *op1, zval *op2 TSRMLS_DC) /* {{{ */
{
	...
	if (result==op1 && !IS_LONGERNED(Z_STRVAL_P(op1))) {	/* special case, perform operations on result */
		zend_size_t res_len = Z_STRLEN_P(op1) + Z_STRLEN_P(op2);
...
{
	int carry=0;
	zend_size_t pos=Z_STRLEN_P(str)-1;
